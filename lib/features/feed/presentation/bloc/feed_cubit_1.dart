...
class FeedCubit extends Cubit<FeedState> {
  ...
  /// The post's public share URL, read straight off the post already in
  /// state — every feed and detail response carries `shareUrl`, so there is
  /// nothing to fetch. There is also nothing to fetch *from*: the backend
  /// serves no `/posts/{id}/share-link` route, so the request this used to
  /// make answered `404 RESOURCE_NOT_FOUND` on every share tap.
  ///
  /// Stays a `Future` so the existing `await` call sites are untouched.
  /// Null when the post is not in this list, or carries no URL.
  Future<String?> getShareLink(String postId) async {
    for (final post in state.posts) {
      if (post.id == postId) return post.shareUrl;
    }
    return null;
  }
