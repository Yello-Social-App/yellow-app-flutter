...
class PostDetailCubit extends Cubit<PostDetailState> {
  ...
  /// The post's public share URL, read off the loaded post rather than
  /// fetched: `shareUrl` arrives with the post itself, and the
  /// `/posts/{id}/share-link` endpoint this used to call does not exist —
  /// it answered `404 RESOURCE_NOT_FOUND` on every share tap.
  ///
  /// Null before the post has loaded, or if it carries no URL; the UI
  /// already handles null with a snackbar.
  Future<String?> getShareLink() async => state.post?.shareUrl;
